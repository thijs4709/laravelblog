...
@section('content')
        <div class="d-flex justify-content-between shadow-lg p-3 mb-5 bg-body-tertiary rounded">
            <div class="d-flex">
                <p class="rounded bg-primary m-0 d-flex align-items-center p-2 text-white">{{$comments->total()}}</p>
                <h1>| Comments</h1>
            </div>
            ...
            <tbody>
                @foreach($comments as $comment)
                    <tr>
                        <td>
                            @if($comment->post)
                                <a href="{{route('comments.show', $comment->post->id)}}">
                                    {{$comment->post->id}}
                                </a>
                            @endif
                        </td>
                        <td>
                            @if($comment->parent)
                            <a href="{{route('comments.show',$comment->parent->id)}}"></a>
                            {{$comment->parent->id}}
                            @endif
                        </td>
                        <td>
                            <a href="{{route('comments.show',$comment->id)}}"></a>
                            {{$comment->id}}
                        </td>
                        <td>
                            ...
                            @else
                                <p class="text-danger">{{$comment->user()->withTrashed()->first() ? $comment->user()->withTrashed()->first() : 'no name'}}</p>
                            @endif
                        </td>
                        <td>
                            {{$comment->body}}
                        ...
                        <td>
                            <div class="nav-item dropdown no-arrow">
                                ...
                                <div class="dropdown-menu shadow"
                                    ...
                                    <a class="dropdown-item" href="{{route('comments.show', $comment)}}"> {{--de $post->slug staat hier bij voor leesbaarheid (kan weg)--}}
